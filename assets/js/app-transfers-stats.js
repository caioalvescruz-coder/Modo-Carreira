...
function abrirLivroCaixa() {
  ...
  $('#livro-caixa-list').innerHTML =
    lc.length === 0
      ? '<div style="text-align:center;padding:20px;color:#94a3b8">Nenhuma movimentação registrada.</div>'
      : lc
          .slice()
          .reverse()
          .map((t) => {
...
function abrirModalNegociar(i) {
  ...
  $('#modal-negociar-nome').innerHTML =
    `${j.primeiroNome} <span style="color:#fbbf24;">${j.sobrenome}</span>`;
...
function confirmarNegocio() {
  ...
  if (tp === 'venda') {
    const val = +$('#modal-negociar-valor-venda').value || 0,
      clu = $('#modal-negociar-clube-venda').value.trim();
    if (!clu) return mostrarToast('Preencha o clube comprador.', true);
    s.verba += val;
    ...
    if (s.lineup)
      for (let k in s.lineup)
        if (
          ...
          delete s.lineup[k];
    mostrarToast(`Venda de ${j.sobrenome} confirmada!`);
  } else {
    const clu = $('#modal-negociar-clube-emp').value.trim();
    if (!clu) return mostrarToast('Preencha o clube de destino.', true);
    ...
    mostrarToast(`Empréstimo de ${j.sobrenome} confirmado!`);
  }
  salvarDados();
  fecharModalNegociar();
  renderizar();
...
function abrirModalContratar(i) {
  ...
  $('#modal-contratar-nome').innerHTML =
    `${j.primeiroNome} <span style="color:#fbbf24;">${j.sobrenome}</span>`;
...
function confirmarContrato() {
  ...
  if (tp === 'compra') {
    const val = +$('#modal-contratar-valor-compra').value || 0,
      clu = $('#modal-contratar-clube-compra').value.trim(),
      sal = +$('#modal-contratar-salario-compra').value || 0;
    if (!clu) return mostrarToast('Preencha o clube vendedor.', true);
    s.verba -= val;
    s.salario -= sal;
    j.salario = sal;
    j.contAnos = +$('#modal-contratar-anos').value || 0;
    j.contMeses = +$('#modal-contratar-meses').value || 0;
    if (j.contAnos === 0 && j.contMeses === 0) j.contMeses = 1;
    j.status = 'Reserva';
    j.num = '-';
    j.situacao = 'Elenco';
    ...
    s.jogadores.push(j);
    src.splice(+idx, 1);
    mostrarToast(`Contratação de ${j.sobrenome} concluída!`);
  } else {
    const clu = $('#modal-contratar-clube-origem').value.trim(),
      sal = +$('#modal-contratar-salario-emp-in').value || 0;
    if (!clu) return mostrarToast('Preencha o clube de origem.', true);
    s.salario -= sal;
    j.salario = sal;
...
function confirmarPromocao() {
  ...
  j.contAnos = +$('#modal-promover-anos').value || 0;
  j.contMeses = +$('#modal-promover-meses').value || 0;
  if (j.contAnos === 0 && j.contMeses === 0) {
    mostrarToast('O contrato precisa ter pelo menos 1 mês de duração.', true);
    return;
  }
...
function renderizarAbaHistorico() {
  ...
  if (!e.historico?.length)
    return (
      topHtml +
      `<div style="text-align:center;padding:40px;color:#94a3b8;font-size:1.2em;">Nenhum histórico disponível ainda.</div></div>`
    );
...
function renderizarSnapshotVisualizacao(idx) {
  ...
  let btn = (a) =>
    `<button style="padding:6px 12px;border-radius:6px;border:1px solid #fbbf24;cursor:pointer;font-weight:700;background:${snapshotAbaSelecionada === a ? '#fbbf24' : 'transparent'};color:${snapshotAbaSelecionada === a ? '#000' : '#fbbf24'};" onclick="mudarAbaSnapshot('${a}')">${a.charAt(0).toUpperCase() + a.slice(1)}</button>`;
  let prmH = '';
  if (sn.premios) {
    const p = sn.premios;
  ...
  if (snapshotAbaSelecionada === 'estatisticas')
    return hTop + renderizarEstatisticas(sn) + `</div>`;
  ...
  return (
    h +
    (!l.length
      ? `<tr><td colspan="10" style="text-align:center;padding:20px;color:#94a3b8;">Nenhum dado salvo.</td></tr>`
      : l
          .map((j) => {
            const stH =
              snapshotAbaSelecionada === 'jogadores'
                ? `<span style="font-weight:700;font-size:0.9em">${{ Titular: 'TIT', Reserva: 'RES', 'Não Relacionado': 'N/R' }[j.status] || 'N/R'}</span>`
                : `<span title="${j.status}" style="font-size:1.2em;">${getStatusEmoji(j.status)}</span>`;
...
const renomearCompExtra = () => {
  ...
  if (compsSelecionadas.includes(editComp))
    compsSelecionadas = compsSelecionadas.map((c) => (c === editComp ? n : c));
  editComp = n;
  salvarDados();
  renderizar();
};
...
function renderizarEstatisticas(sourceSeason = null) {
  ...
  const comps = [
    ...new Set([...Object.keys(getCompC()), ...(s.calendario || []).map((c) => c.campeonato)]),
  ];
  ...
  (s.calendario || []).forEach((c) => {
    if (
      compsSelecionadas.includes(c.campeonato) &&
      c.gp !== '' &&
      c.gc !== '' &&
      c.gp != null &&
      c.gc != null
  ...
  let flt = uniqueJogadores.filter((j) =>
  ...
  let pStats = flt.map((j) => {
    ...
    compsSelecionadas.forEach((c) => {
      if (j.est && j.est[c]) {
        ...
        st.cv += j.est[c].cv || 0;
        let m =
          j.est[c].media !== undefined
            ? parseFloat(j.est[c].media)
            : j.est[c].jogos
              ? (j.est[c].somaNotas || 0) / j.est[c].jogos
              : 0;
  ...
  pStats.sort((a, b) => {
    ...
    if (
      ...
      vA =
        sortStatsField === 'media'
          ? +a.media
          : sortStatsField === 'impacto'
            ? +a.impacto
            : a[sortStatsField];
      vB =
        sortStatsField === 'media'
  ...
  if (!isRead)
    html += `<div style="text-align:center;margin-bottom:15px;display:flex;justify-content:center;align-items:center;gap:10px;flex-wrap:wrap;"><label style="color:#fbbf24;font-weight:700;">Editando Competição:</label><select id="stat-edit-comp" onchange="editComp=this.value; renderizar()" style="padding:6px;border-radius:6px;background:#0f172a;color:#fff;border:1px solid #fbbf24;">${comps.map((c) => `<option value="${c}" ${c === editComp ? 'selected' : ''}>${c}</option>`).join('')}</select><button onclick="abrirModalCompeticoes()" style="background:#15803d;color:#fff;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-weight:700;margin-left:10px;">Gerenciar Competições</button></div>`;

  const th = (f, l) =>
    `<th onclick="sortStats('${f}')" style="cursor:pointer;white-space:nowrap;${f === 'sobrenome' ? 'text-align:left' : ''}">${l} ${sortStatsField === f ? (sortStatsDir === 'desc' ? '⬇️' : '⬆️') : ''}</th>`;
  ...
  html += pStats
    .map((st) => {
      ...
      let dH =
        dO > 0
          ? `<span style="color:#10b981;font-weight:900;">+${dO}</span>`
          : dO < 0
      ...
      if (!isRead) {
        ...
        let med =
          jEst.media !== undefined
            ? parseFloat(jEst.media).toFixed(1)
            : jEst.jogos
              ? (jEst.somaNotas / jEst.jogos).toFixed(1)
              : '0.0';
        const inp = (f) =>
