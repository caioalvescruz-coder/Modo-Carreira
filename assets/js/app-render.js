function renderizar(skipF = false) {
  ...
  const replaceGL = (j) => {
    if (j.pos === 'GL') j.pos = 'GOL';
    if (j.positions && Array.isArray(j.positions))
      j.positions = j.positions.map((p) => (p === 'GL' ? 'GOL' : p));
  };
  if (s && s.jogadores) s.jogadores.forEach(replaceGL);
  if (s && s.mercado) s.mercado.forEach(replaceGL);
  if (window.bdJogadores) window.bdJogadores.forEach(replaceGL);
  if (window.InboxEngine && window.InboxEngine.migrateLegacyEmails) {
    window.InboxEngine.migrateLegacyEmails();
  }

  $('#squad-select').innerHTML = appData.elencos
    .map(
      (e, i) =>
        `<option value="${i}" ${i === appData.indiceAtivo ? 'selected' : ''}>${e.nome}</option>`,
    )
    .join('');
  if ($('#season-select'))
    $('#season-select').innerHTML = elAtv.temporadas
      .map(
        (t, i) =>
          `<option value="${i}" ${i === elAtv.temporadaAtiva ? 'selected' : ''}>${t.nome}</option>`,
      )
      .join('');
  $('#main-title').innerHTML = modoEdicao
    ? `<input type="text" id="edit-squad-name" value="${elAtv.nome}" style="text-align:center;background:transparent;color:#fbbf24;border:none;border-bottom:2px solid #fbbf24;font-size:1em;width:300px;">`
    : elAtv.nome;
  ...
  const isO =
    ...
    abaAtiva === 'inboxmaster';
  const isBanco = abaAtiva === 'banco';
  $$('.fixed-buttons, .fixed-buttons-bottom').forEach(
    (e) => (e.style.display = abaAtiva === 'historico' || isBanco ? 'none' : 'flex'),
  );
  $('#filtros-container').style.display = isO ? 'none' : 'block';
  $('#dashboard').style.display = isO || isBanco ? 'none' : 'grid';
  if ($('#inbox-master-container')) $('#inbox-master-container').style.display = 'none';

  c.classList.remove('grid');
  if (!modoTabela && !isO && abaAtiva !== 'estatisticas') c.classList.add('grid');
  c.style.display = modoTabela || isO || abaAtiva === 'estatisticas' ? 'block' : 'grid';
  if ($('.sort-area'))
    $('.sort-area').style.display = abaAtiva === 'estatisticas' ? 'none' : 'flex';

  if ($('#btn-adicionar'))
    $('#btn-adicionar').style.display =
      modoEdicao && (abaAtiva === 'jogadores' || abaAtiva === 'mercado' || abaAtiva === 'base')
        ? 'flex'
        : 'none';
  if ($('#btn-excluir-tudo'))
    $('#btn-excluir-tudo').style.display =
      modoEdicao && (abaAtiva === 'jogadores' || abaAtiva === 'mercado' || abaAtiva === 'base')
        ? 'flex'
        : 'none';
  if ($('#btn-selecionar-todos'))
    $('#btn-selecionar-todos').style.display =
      modoEdicao && (abaAtiva === 'jogadores' || abaAtiva === 'mercado' || abaAtiva === 'base')
        ? 'flex'
        : 'none';
  if ($('#btn-excluir-visiveis'))
    $('#btn-excluir-visiveis').style.display =
      modoEdicao && (abaAtiva === 'jogadores' || abaAtiva === 'mercado' || abaAtiva === 'base')
        ? 'flex'
        : 'none';
  if ($('#btn-excluir-selecionados')) {
    $('#btn-excluir-selecionados').style.display =
      modoEdicao &&
      (abaAtiva === 'jogadores' || abaAtiva === 'mercado' || abaAtiva === 'base') &&
      selCards.length > 0
        ? 'flex'
        : 'none';
    if ($('#count-sel')) $('#count-sel').innerText = selCards.length;
  }
  if ($('#btn-avancar-temporada'))
    $('#btn-avancar-temporada').style.display =
      modoEdicao && abaAtiva === 'jogadores' ? 'flex' : 'none';
  $$('.squad-selector .btn-add').forEach(
    (b) => (b.style.display = modoEdicao ? 'inline-block' : 'none'),
  );
  $$('.squad-selector .btn-del').forEach(
    (b) => (b.style.display = modoEdicao ? 'inline-block' : 'none'),
  );
  if ($('#btn-exportar-elenco-json'))
    $('#btn-exportar-elenco-json').style.display = modoEdicao ? 'inline-block' : 'none';
  if ($('#btn-importar-elenco-json'))
    $('#btn-importar-elenco-json').style.display = modoEdicao ? 'inline-block' : 'none';
  ...
  if (abaAtiva === 'calendario') {
    c.innerHTML = renderizarCalendario();
    return;
  }
  if (abaAtiva === 'historico') {
    c.innerHTML = renderizarAbaHistorico();
    return;
  }
  if (abaAtiva === 'taticas') {
    c.innerHTML = renderizarTaticas();
    return;
  }
  if (abaAtiva === 'estatisticas') {
    c.innerHTML = renderizarEstatisticas();
    return;
  }
  if (abaAtiva === 'inboxmaster') {
    if (typeof InboxUI !== 'undefined') InboxUI.renderTab(InboxUI.activeFolder);
    return;
  }
  ...
  if ($('#btn-filtro-renovacao'))
    $('#btn-filtro-renovacao').classList.toggle('ativo', filtroRenovacaoAtivo);
  ...
  const sO =
    abaAtiva === 'estatisticas'
      ? ['Elenco', 'Emprest. - IN']
      : abaAtiva === 'jogadores'
        ? ['Elenco', 'Emprest. - IN', 'Emprest. - OUT', 'Vender', 'Emprestar']
        : abaAtiva === 'base'
          ? ['Base']
          : subAbaMercado !== 'alvos'
            ? ['Elenco', 'Emprest. - IN', 'Emprest. - OUT', 'Vender', 'Emprestar']
            : ['Observação', 'Empréstimo', 'Transferência'];
  const stO =
    abaAtiva === 'mercado'
      ? ['Prioridade', 'Alternativa', 'Fim de Contrato']
      : ['Titular', 'Reserva', 'Não Relacionado'];

  if (!skipF) {
    const btnL = (t) =>
      `<button class="filtro-btn" style="border:none;background:transparent;padding:0 2px;font-size:.7em;margin-right:0;opacity:.6" onclick="filtros['${t}']=${t === 'pe' ? "''" : '[]'};renderizar()" title="Limpar">🧹</button>`;
    $('#filtro-pos').innerHTML =
      `${btnL('pos')}<span class="filtro-titulo">Posição:</span>` +
      pO
        .map(
          (o) =>
            `<button class="filtro-btn ${filtros.pos.includes(o) ? 'ativo' : ''}" onclick="window.toggleFiltro('pos','${o}')">${o}</button>`,
        )
        .join('');
    $('#filtro-pe').innerHTML =
      `${btnL('pe')}<span class="filtro-titulo">Pé Preferido:</span>` +
      ['Ambos', 'Destro', 'Canhoto']
        .map(
          (o) =>
            `<button class="filtro-btn ${filtros.pe === (o === 'Destro' ? 'Dir.' : o === 'Canhoto' ? 'Esq.' : o) ? 'ativo' : ''}" onclick="window.mudarFiltroPe('${o === 'Destro' ? 'Dir.' : o === 'Canhoto' ? 'Esq.' : o}')">${o}</button>`,
        )
        .join('');
    $('#filtro-sit').innerHTML =
      abaAtiva === 'base'
        ? ''
        : `${btnL('sit')}<span class="filtro-titulo">Situação:</span>` +
          sO
            .filter((o) => !(abaAtiva === 'jogadores' && o === 'Emprest. - OUT'))
            .map(
              (o) =>
                `<button class="filtro-btn ${filtros.sit.includes(o) ? 'ativo' : ''}" onclick="window.toggleFiltro('sit','${o}')">${lblSit(o)}</button>`,
            )
            .join('');
    $('#filtro-status').innerHTML =
      `${btnL('status')}<span class="filtro-titulo">Status:</span>` +
      stO
        .map(
          (o) =>
            `<button class="filtro-btn ${filtros.status.includes(o) ? 'ativo' : ''}" onclick="window.toggleFiltro('status','${o}')">${lblSt(o)}</button>`,
        )
        .join('');

    $('#filtro-pos').className = 'filtro-linha' + (filtros.pos.length ? ' ativo-linha' : '');
    $('#filtro-pe').className = 'filtro-linha' + (filtros.pe ? ' ativo-linha' : '');
    $('#filtro-sit').className = 'filtro-linha' + (filtros.sit.length ? ' ativo-linha' : '');
    $('#filtro-sit').style.display =
      abaAtiva === 'base' || (abaAtiva === 'mercado' && subAbaMercado === 'out') ? 'none' : 'flex';
    $('#filtro-status').className = 'filtro-linha' + (filtros.status.length ? ' ativo-linha' : '');
    $('#filtro-status').style.display =
      abaAtiva === 'mercado' && subAbaMercado === 'out' ? 'none' : 'flex';
  ...
  let f = srcArr.filter(
    (j) =>
        ...
        : abaAtiva === 'base'
          ...
          : abaAtiva === 'jogadores'
            ...
            : abaAtiva === 'mercado'
              ? subAbaMercado === 'alvos'
                ? true
                : subAbaMercado === 'vendas'
                  ? j.situacao === 'Vender'
                  : subAbaMercado === 'emprestar'
                    ? j.situacao === 'Emprestar'
                    : j.situacao === 'Emprest. - OUT'
              : true) &&
      (!filtros.pe || (filtros.pe === 'Ambos' ? j.pr === 5 : j.pe === filtros.pe)) &&
      ...
      (!tc ||
        (j.clube &&
          tc.split(' ').every((w) => w.trim() === '' || j.clube.toLowerCase().includes(w)))) &&
      (!filtroRenovacaoAtivo || (+j.contAnos === 0 && +j.contMeses <= 6)) &&
      j.idade >= filtros.fx.idade[0] &&
      j.idade <= filtros.fx.idade[1] &&
      j.ovr >= filtros.fx.ovr[0] &&
      j.ovr <= filtros.fx.ovr[1] &&
      j.pot >= filtros.fx.pot[0] &&
      j.pot <= filtros.fx.pot[1] &&
      j.valor >= filtros.fx.valor[0] &&
      j.valor <= filtros.fx.valor[1],
  );
  ...
  const itensPorPagina = modoTabela ? 50 : 20;
  ...
  if (totalPaginas > 1) {
    ...
    if (paginaAtual > 1)
      pB.push(
        `<button class="filtro-btn" style="padding:6px 10px;font-weight:bold" onclick="mudarPagina(${paginaAtual - 1})">« Anterior</button>`,
      );
    ...
    for (let p = start; p <= end; p++)
      pB.push(
        `<button class="filtro-btn ${p === paginaAtual ? 'ativo' : ''}" onclick="mudarPagina(${p})">${p}</button>`,
      );
    if (end < totalPaginas - 1) pB.push('<span style="color:#94a3b8">...</span>');
    if (end < totalPaginas)
      pB.push(
        `<button class="filtro-btn" onclick="mudarPagina(${totalPaginas})">${totalPaginas}</button>`,
      );
    if (paginaAtual < totalPaginas)
      pB.push(
        `<button class="filtro-btn" style="padding:6px 10px;font-weight:bold" onclick="mudarPagina(${paginaAtual + 1})">Próximo »</button>`,
      );
  ...
  if (modoEdicao) {
    ...
    bulkHtml = `<div style="display:flex; justify-content:space-between; align-items:center; background:#1e293b; padding:10px; border-radius:8px; margin-bottom:15px; border:1px solid #334155;">
            <button class="filtro-btn" onclick='selecionarTodosVisiveisDaAba()' style="margin:0;">☑️ Selecionar Todos Visíveis</button>
            ${selCards.length >= 2 ? `<button style="background:#b91c1c; color:white; border:none; padding:8px 15px; border-radius:5px; font-weight:bold; cursor:pointer;" onclick="excluirSelecionados()">🗑️ Excluir ${selCards.length} selecionados</button>` : ''}
        </div>`;
  }
  ...
  if (modoTabela) {
        ...
        : paginados
            .map((j) => {
              ...
              const i = srcArr.indexOf(j),
                ...
                stHtml =
                  abaAtiva === 'jogadores'
                    ? `<span style="font-weight:700;font-size:0.9em">${mapSt[j.status] || 'N/R'}</span>`
                    : `<span style="font-size:1.2em;">${getStatusEmoji(j.status)}</span>`,
                tblImg = j.img
    ...
    c.innerHTML =
      ...
      paginationHtml +
      (f.length === 0
        ? `<div style="grid-column:1/-1;text-align:center;padding:40px;color:#94a3b8;font-size:1.1em;font-weight:bold;background:#0f172a;border-radius:8px;border:1px dashed #334155;margin-top:20px;">Nenhum jogador listado sob estes critérios.</div>`
        : paginados
            .map((j) => {
              inicializarBases(j);
              ...
              const i = srcArr.indexOf(j),
                ...
                lb =
                  j.pos === 'GOL'
                    ? ['ELA', 'MAN', 'CHU', 'REF', 'VEL', 'POS']
                    : ['RIT', 'FIN', 'PAS', 'CON', 'DEF', 'FIS'];
              const bS = (cp, v, mn, mx) =>
                `<div style="display:flex;justify-content:center;align-items:center;gap:5px;margin-top:2px;"><button class="btn-inc btn-square" onclick="alterarValor(${i},'${cp}',-1,null,${mn},${mx})">-</button><span style="font-weight:900;cursor:pointer;" onclick="transformarEmInput(this,${i},'${cp}',null,${v})">${v}</span><button class="btn-inc btn-square" onclick="alterarValor(${i},'${cp}',1,null,${mn},${mx})">+</button>${renderDeltaSpan(j, cp, null)}</div>`;
              const isM = abaAtiva === 'mercado',
                ...
                pCls =
                  j.pos === 'GOL'
                    ? 'c-gol'
                    : ['ZAG', 'LE', 'LD'].includes(j.pos)
                      ? 'c-def'
                      : ['VOL', 'MC', 'MEI', 'MD', 'ME'].includes(j.pos)
                        ? 'c-mid'
                        : 'c-atk';
              const imgH = `<div class="avatar-wrapper" ${modoEdicao ? `onclick="abrirUpload(${i})"` : ''}>${j.img ? `<img src="${j.img}" class="avatar-img" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='assets/camisa.png';">` : defSvg}${modoEdicao ? `<div class="avatar-edit-icon">📷</div>` : ''}</div>`;
              let cb = `Contrato: ${j.contAnos}a ${j.contMeses}m`;
              if (isOut)
                cb = `Emprestado para <b>${j.empClube || 'Desconhecido'}</b><br>Prazo: ${j.empAnos}a ${j.empMeses}m <br><span style="color:#fbbf24">${j.empOpcaoCompra ? `Opção: ${formatMoney(j.empValorCompra)}` : 'Sem Opção de Compra'}</span>`;
              else if (j.situacao === 'Emprest. - IN')
                cb = `Emprestado de <b>${j.empClube || 'Desconhecido'}</b><br>Prazo: ${j.contAnos}a ${j.contMeses}m <br><span style="color:#fbbf24">${j.empOpcaoCompra ? `Opção: ${formatMoney(j.empValorCompra)}` : 'Sem Opção de Compra'}</span>`;
...
${!modoEdicao && abaAtiva === 'mercado' && subAbaMercado === 'alvos' ? `<button onclick="abrirModalContratar(${i})" style="position:absolute;top:5px;right:5px;background:#15803d;color:#fff;border:none;padding:4px 8px;border-radius:4px;font-weight:700;cursor:pointer;font-size:.8em;z-index:10;">🤝 Contratar</button>` : window.getAlvoBtnHtml(i, j)}
...
function excluirSelecionados() {
  ...
  if (
    !confirm(
      `Você está prestes a excluir definitivamente ${selCards.length} jogadores selecionados. Tem certeza?`,
    )
  )
    return;
    ...
    if (src === getSeason().jogadores) {
      ...
      for (let k in l) {
        if (
          l[k] === r ||
          l[k] === r.id ||
          l[k] === r.primeiroNome + r.sobrenome ||
          l[k]?.id === r.id
        ) {
          delete l[k];
        }
...
function excluirVisiveis() {
  ...
  if (
    !confirm(
      `Você está prestes a excluir definitivamente ${f.length} jogadores listados na tela. Tem certeza?`,
    )
  )
    return;
...
function exportarElencoAtualJSON() {
  try {
    const elencoAtual = appData.elencos[appData.indiceAtivo];
    const exportData = {
      ...appData,
      elencos: [elencoAtual],
      indiceAtivo: 0,
    ...
    const a = document.createElement('a');
    a.href = url;
    let safeName = elencoAtual.nome.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    a.download = `manager_fc_${safeName}_${new Date().toISOString().split('T')[0]}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  } catch (e) {
    alert('Erro ao exportar elenco atual.');
  }
}

function importarElencoAtualJSON(event) {
  ...
  reader.onload = async function (e) {
    try {
      ...
      if (!parsed.elencos || !Array.isArray(parsed.elencos) || parsed.elencos.length === 0)
        throw new Error('JSON de elenco inválido.');
      if (
        confirm(
          'ATENÇÃO: isto irá SOBRESCREVER o seu elenco atual inteiro! Os outros elencos e saves permanecerão intactos. Deseja continuar?',
        )
      ) {
        appData.elencos[appData.indiceAtivo] = parsed.elencos[parsed.indiceAtivo || 0];
        appData.schemaVersion = 2; // Garantir schema atualizado

        // Forçar sync para o backend (simulando salvarDados ou managerApp.queueSave)
        let str = JSON.stringify(appData);
        if (typeof atualizarStorageInfo === 'function') atualizarStorageInfo(str);
        if (window.ManagerAuth && window.ManagerAuth.queueSave) {
          window.ManagerAuth.queueSave(appData);
        } else if (typeof salvarDados === 'function') {
          salvarDados(true);
        }

        // Atualizar tela
        if (typeof trocarElenco === 'function') trocarElenco();
        else renderizar();

        window.mostrarToast?.('Elenco atual atualizado com sucesso.');
      }
    } catch (err) {
      console.error(err);
      alert('Erro ao ler ou salvar o elenco. Verifique se o JSON é válido.');
    }
    event.target.value = '';
  };
...
function importarBackup(event) {
  ...
  reader.onload = async function (e) {
    try {
      ...
      if (
        confirm(
          'ATENÇÃO: importar um backup substituirá TODO o progresso deste usuário. Deseja continuar?',
        )
      ) {
      ...
      console.error(err);
      alert(
        'Erro ao ler ou salvar o backup. Verifique se o JSON é válido e se há conexão com a internet.',
      );
    }
    event.target.value = '';
  };
...
function verificarEventosNarrativos(s) {
  ...
  if (isUltimoDia) {
    ...
    if (!jaTeve(pid)) {
      addNotif(
        pid,
        `Diretoria Financeira e Técnica: Hoje é o último dia do mês. Lembre-se de acessar o EA FC e atualizar os Valores de Mercado, Salários, Overall (OVR), Potencial (POT) e atributos de todos os jogadores que evoluíram.`,
      );
      registrar(pid);
    }
  }
...
`;
      ...
      let defs = s.jogadores.filter(
        (j) => ['ZAG', 'LD', 'LE'].includes(j.pos) && j.situacao === 'Elenco',
      ).length;
      let meias = s.jogadores.filter(
        (j) => ['VOL', 'MC', 'MEI', 'MD', 'ME'].includes(j.pos) && j.situacao === 'Elenco',
      ).length;
      let atqs = s.jogadores.filter(
        (j) => ['ATA', 'SA', 'PE', 'PD'].includes(j.pos) && j.situacao === 'Elenco',
      ).length;

      if (defs < 6)
        relatorio += `- Defesa: Temos poucos defensores de ofício no elenco principal. Considere buscar reforços no Mercado.
`;
      if (meias < 6)
        relatorio += `- Meio-campo: Precisamos de mais opções no setor criativo ou de marcação.
`;
      if (atqs < 4)
        relatorio += `- Ataque: Falta profundidade no ataque, um lesão pode nos complicar.
`;

      // Jogadores sem espaço / má fase (Baixa média de notas)
      // Agrega estatísticas de todas as competições para análise narrativa
      let jogadoresComNota = s.jogadores
        .filter((j) => j.situacao === 'Elenco')
        .map((j, idx) => {
          let numJogos = 0,
            somaMedia = 0;
          if (j.est)
            Object.values(j.est).forEach((e) => {
              numJogos += e.jogos || 0;
...
`;
      ...
      let reservasPedindoPassagem = jogadoresComNota.filter(
        (x) => x.media >= 7.5 && x.numJogos < 5 && x.j.idade <= 23,
      );
...
`;
  ...
  let ultimosJogos = (s.calendario || [])
    .filter((e) => e.relatorio)
  ...
  if (ultimosJogos.length === 3) {
    ...
    if (derrotas === 3) {
      ...
      if (!jaTeve(pid)) {
        addNotif(
          pid,
          `Cobrança da Diretoria: Três derrotas seguidas. O time precisa reagir imediatamente para não comprometer os objetivos da temporada.`,
        );
        registrar(pid);
      }
    }
    ...
    if (cleanSheets === 3) {
      ...
      if (!jaTeve(pid)) {
        addNotif(
          pid,
          `Imprensa Esportiva: A equipe não sofreu gols nos últimos 3 jogos. O sistema defensivo vem sendo muito elogiado.`,
        );
        registrar(pid);
      }
    }
  ...
  s.jogadores.forEach((j, i) => {
    if (j.situacao === 'Elenco') {
      ...
      let totalJogosRumor = 0;
      if (j.est) Object.values(j.est).forEach((e) => (totalJogosRumor += e.jogos || 0));
      let mediaRumor = 0;
      if (j.est) {
        let sm = 0;
        Object.values(j.est).forEach((e) => {
          sm += (parseFloat(e.media) || 0) * (e.jogos || 1);
        });
        mediaRumor = totalJogosRumor > 0 ? sm / totalJogosRumor : 0;
      }
      if (totalJogosRumor >= 5 && mediaRumor >= 8.0) {
        ...
        if (!jaTeve(pid)) {
          addNotif(
            pid,
            `Rumor de Mercado: As atuações brilhantes de ${j.primeiroNome} ${j.sobrenome} despertaram o interesse de clubes do exterior. Fique atento a propostas.`,
          );
          registrar(pid);
        }
      }
    ...
    if (j.situacao === 'Base' && j.potMax >= 90) {
      ...
      if (!jaTeve(pid)) {
        addNotif(
          pid,
          `Olheiro da Base: Temos um garoto de ouro. ${j.primeiroNome} ${j.sobrenome} tem potencial para se tornar um dos melhores do mundo. De olho nele!`,
        );
        registrar(pid);
      }
    }
...
window.comprimirTudo = async function () {
  if (
    !confirm(
      'Isso vai comprimir todas as imagens de todos os elencos e temporadas para liberar espaço. A qualidade vai diminuir um pouco, mas evitará que o site trave. Deseja continuar?',
    )
  )
    return;
...
window.adicionarAoAlvoUniversal = async function (idx, btn) {
  ...
  const season = elAtv.temporadas[elAtv.temporadaAtiva || 0];
...
window.selecionarTodosVisiveisDaAba = function () {
  const s = getSeason();
  const srcArr =
    abaAtiva === 'mercado' ? (subAbaMercado === 'alvos' ? s.mercado : s.jogadores) : s.jogadores;

  // Reproduce the filter logic to get exactly what's visible
  const tm = document.getElementById('search-input')
    ? document.getElementById('search-input').value.toLowerCase()
    : '';
  const tc = document.getElementById('search-clube')
    ? document.getElementById('search-clube').value.toLowerCase()
    : '';
  const tcList = tc
    .split(' ')
    .map((w) => w.trim())
