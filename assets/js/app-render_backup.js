function renderizar(skipF = false) {
  ...
  if (!skipF) {
    const btnL = (t) =>
      `<button class="filtro-btn" style="border:none;background:transparent;padding:0 2px;font-size:.7em;margin-right:0;opacity:.6" onclick="filtros['${t}']=[];renderizar()" title="Limpar">✖️</button>`;
    $('#filtro-pos').innerHTML =
      `${btnL('pos')}<span class="filtro-titulo">Posição:</span>` +
      pO
        .map(
          (o) =>
            `<button class="filtro-btn ${filtros.pos.includes(o) ? 'ativo' : ''}" onclick="toggleFiltro('pos','${o}')">${o}</button>`,
        )
        .join('');
      ...
      abaAtiva === 'base'
        ...
        : `${btnL('sit')}<span class="filtro-titulo">Situação:</span>` +
          sO
            ...
            .map(
              (o) =>
                `<button class="filtro-btn ${filtros.sit.includes(o) ? 'ativo' : ''}" onclick="toggleFiltro('sit','${o}')">${lblSit(o)}</button>`,
            )
            .join('');
    $('#filtro-status').innerHTML =
      ...
      stO
        .map(
          (o) =>
            `<button class="filtro-btn ${filtros.status.includes(o) ? 'ativo' : ''}" onclick="toggleFiltro('status','${o}')">${lblSt(o)}</button>`,
        )
        .join('');
...
<div class="header" style="flex-direction:column;gap:5px;align-items:stretch"><div style="display:flex;width:100%;align-items:center;gap:5px">${modoEdicao ? `<input type="checkbox" style="transform:scale(1.3);cursor:pointer;margin-right:2px" ${selCards.includes(i) ? 'checked' : ''} onclick="toggleSel(${i})">` : ''}${imgH}<div class="nome">${modoEdicao ? `<input type="text" data-field="primeiroNome" class="name-input" value="${j.primeiroNome}">` : j.primeiroNome}</div><div class="sobrenome">${modoEdicao ? `<input type="text" data-field="sobrenome" class="name-input" value="${j.sobrenome}">` : j.sobrenome}</div></div><div style="display:flex;width:100%;align-items:center"><div style="flex:1;text-align:left"><div class="pos-stat-box ${stCls}" style="border:none;padding:2px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${stTxt}</div></div><div style="flex:1;text-align:center;display:flex;flex-direction:column;align-items:center;gap:2px;"><div class="pos-stat-box ${pCls}" style="border:none;margin-bottom:0;min-width:35px">${modoEdicao ? `<select data-field="pos" style="width:55px;background:transparent;color:inherit;border:none;font-weight:inherit">${pO.map((o) => `<option ${j.pos === o ? 'selected' : ''}>${o}</option>`).join('')}</select>` : j.pos}</div>${
                !modoEdicao && j.positions && j.positions.length > 1
                  ? `<div style="display:flex;gap:2px;flex-wrap:wrap;justify-content:center;margin-top:2px">${j.positions
                      ...
                      .map(
                        (p) =>
                          `<span style="font-size:0.65em;background:rgba(255,255,255,0.1);padding:1px 4px;border-radius:3px">${p}</span>`,
                      )
                      .join('')}</div>`
                  : modoEdicao
    ...
    if (src === getSeason().jogadores) {
      ...
      for (let k in l) {
        if (
          ...
          s.lineup[k] === r.primeiroNome + r.sobrenome ||
          l[k]?.id === r.id ||
          s.lineup[k]?.id === r.id
        ) {
          delete l[k];
