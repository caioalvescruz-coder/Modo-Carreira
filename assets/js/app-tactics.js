function renderizarTaticas() {
  ...
  const lH = sq
    .map((j) => {
      ...
      return `<div class="taticas-list-item" ${isSusp ? 'style="border:1px solid #ef4444;background:#2d0000;"' : ''} draggable="${!isSusp}" ondragstart="${!isSusp ? `dragT(event,'L',${s.jogadores.indexOf(j)})` : 'return false'}"><div>${j.pos}</div><div style="font-weight:700">${j.primeiroNome ? j.primeiroNome + ' ' : ''}${j.sobrenome} ${isSusp ? '🟥' : ''}</div><div>${j.idade}</div><div><div class="stat-box ${getBg(j.ovr)}" style="padding:2px 4px;border-radius:3px;">${j.ovr}</div></div><div><div class="stat-box ${getBg(j.pot)}" style="padding:2px 4px;border-radius:3px;">${j.pot}</div></div><div>${selSt}</div></div>`;
    })
    .join('');
  let sOVR = {
    DEF: {
