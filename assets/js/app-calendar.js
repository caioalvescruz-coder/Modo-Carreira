...
window.renderRelatorio = () => {
  ...
  $('#rel-body').innerHTML =
    `<div class="modal-row"><label>Partida Pendente:</label><select id="rel-evt" style="background:#0f172a;color:#fff;padding:8px;border:1px solid #3b82f6;border-radius:6px;font-weight:bold">${m.map((e) => `<option value="${e.id}">${e.data.split('-').reverse().join('/')} - ${e.adversario} (${e.campeonato})</option>`).join('')}</select></div><div class="modal-row" style="flex-direction:row;justify-content:center;align-items:center;gap:15px;background:#0f172a;padding:10px;border-radius:8px;border:1px solid #1e293b"><div style="text-align:center;color:#10b981;font-weight:bold">Nós<br><input type="number" id="rel-gp" value="${st.gp}" min="0" style="width:60px;text-align:center;font-size:1.2em;margin-top:5px;background:#1e293b;color:#fff;border:1px solid #334155;border-radius:4px" onchange="window.relSt.gp=this.value"></div><div style="font-weight:900;font-size:1.5em;color:#64748b">X</div><div style="text-align:center;color:#ef4444;font-weight:bold">Adv.<br><input type="number" id="rel-gc" value="${st.gc}" min="0" style="width:60px;text-align:center;font-size:1.2em;margin-top:5px;background:#1e293b;color:#fff;border:1px solid #334155;border-radius:4px" onchange="window.relSt.gc=this.value"></div></div><div style="color:#fbbf24;font-weight:900;text-transform:uppercase;font-size:.8em;letter-spacing:1px;margin-top:5px">Titulares (${st.t.length})</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;">${st.t.map((i) => pR(i, !1)).join('')}</div><div style="color:#fbbf24;font-weight:900;text-transform:uppercase;font-size:.8em;letter-spacing:1px;margin-top:5px;display:flex;justify-content:space-between"><span>Reservas</span><span style="color:#94a3b8">${st.s.length}/5</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;">${st.s.map((i) => pR(i, !0)).join('')}${st.s.length < 5 ? `<select onchange="if(this.value!=='') addRelSub(this.value)" style="background:#1e293b;color:#fff;padding:8px;border:1px dashed #fbbf24;border-radius:6px;cursor:pointer"><option value="">+ Adicionar Reserva...</option>${avS.map((i) => `<option value="${i}">${s.jogadores[i].pos} - ${s.jogadores[i].primeiroNome} ${s.jogadores[i].sobrenome}</option>`).join('')}</select>` : ''}</div>`;
};
