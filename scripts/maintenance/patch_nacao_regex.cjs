...
const regex = /const nacaoH = j\.nacionalidade \? (.*?);/s;

const replacement = "const nacaoH = j.nacionalidade ? `<div style=\"font-size:0.75em;color:#94a3b8;margin-top:2px;display:flex;align-items:center;justify-content:center;gap:3px\">${j.flagUrl ? \\`<img src=\"${j.flagUrl}\" style=\"width:16px;height:12px;border-radius:2px\" title=\"${j.nacionalidade}\">\\` : \\'Nac:\\'} ${j.nacionalidade}</div>` : `<div style=\"font-size:0.75em;color:#94a3b8;margin-top:2px;display:flex;align-items:center;justify-content:center;gap:3px\">Nac: 🚫</div>`;";

r = r.replace(regex, replacement);
fs.writeFileSync('assets/js/app-render.js', r);
console.log('App-render regex patched!');
