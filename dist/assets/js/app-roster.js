function processarImg(e) {
  ...
  r.onload = (ev) => {
    ...
    img.onload = () => {
      ...
      ctx.fillRect(0, 0, w, h); // fallback background for transparent pngs
...
async function mudarAba(a) {
  ...
  if (a === 'banco' && window.loadDatabase && !window.databaseLoaded) {
  ...
  r.onload = (ev) => {
    ev.target.result
      ...
      .forEach((row) => {
        ...
        if (c.length >= 24)
          getSeason()[abaAtiva === 'mercado' ? 'mercado' : 'jogadores'].push({
            primeiroNome: c[0],
            sobrenome: c[1],
            num: c[2],
            pos: c[3],
    ...
    renderizar();
    alert(
      'Importação concluída para a aba ' +
        (abaAtiva === 'mercado' ? 'Mercado' : abaAtiva === 'base' ? 'Base' : 'Elenco') +
        '!',
    );
  };
    ...
    ev.target.result
      ...
      .forEach((row) => {
        ...
        if (c.length >= 5 && c[0] && c[1]) {
          let dataStr = c[0].trim();
          if (dataStr.includes('/')) {
            const parts = dataStr.split('/');
            if (parts.length === 3) dataStr = `${parts[2]}-${parts[1]}-${parts[0]}`;
          }
          let advStr = c[1].trim();
          let mandoStr = c[2] ? c[2].trim() : 'C';
          let rodadaStr = c[3] ? c[3].trim() : '';
          let campStr = c[4] ? c[4].trim() : '';
          let gpStr = c[5] ? c[5].trim() : '';
          let gcStr = c[6] ? c[6].trim() : '';
          
          let existente = cal.find(x => x.data === dataStr && x.adversario === advStr);
          if (existente) {
            if (mandoStr) existente.mando = mandoStr;
            if (rodadaStr) existente.rodada = rodadaStr;
            if (campStr) existente.campeonato = campStr;
            if (gpStr !== '') existente.gp = gpStr;
            if (gcStr !== '') existente.gc = gcStr;
          } else {
            cal.push({
...
window.exportarCalendarioCSV = function() {
  const cal = getSeason().calendario || [];
  if (cal.length === 0) {
    alert('Calendário vazio! Não há o que exportar.');
    return;
  }
  let csvContent = "Data,Adversário,Mando,Rodada,Campeonato,GP,GC\n";
  cal.forEach(e => {
    let d = e.data;
    if (d && d.includes('-')) {
      const p = d.split('-');
    ...
    const gp = (e.gp !== undefined && e.gp !== null) ? e.gp : '';
    const gc = (e.gc !== undefined && e.gc !== null) ? e.gc : '';
    csvContent += `${d},${e.adversario || ''},${e.mando || 'C'},${e.rodada || ''},${e.campeonato || ''},${gp},${gc}\n`;
  });
...
window.adicionarJogador = function () {
  try {
    ...
    console.error(e);
    mostrarToast('Erro ao adicionar jogador', true);
  }
};
...
function alterarValor(i, c, d, s = null, mn = 1, mx = 99) {
  ...
  else if (['multa', 'valor', 'salario'].includes(c))
    j[c] = Math.max(0, parseInt(j[c] || 0) + d * 1e5);
